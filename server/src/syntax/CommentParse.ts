...
export class CommentParse {
    ...
    public computeText(position: Position): ICommentBlock | null {
        function iscommentTranslateBaidu(scopes: string[]) {
            //评论的token标记
            let arr = [
        ...
        function skipcommentTranslateBaidu(scope: string) {
        ...
        if (scopes && iscommentTranslateBaidu(scopes)) {
            return this.multiScope({
                positionLine: position.line,
                dataTokens1: data.tokens1,
                token1Index
            }, iscommentTranslateBaidu, this._model.length - 1, 0, skipcommentTranslateBaidu);
        }
        return null;
    }
}